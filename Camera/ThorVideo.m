...
image = GetImage2(hCam,width,height);
...
uicontrol('Style','pushbutton','String','Save Image','Position',[600 720 100 30],'Callback',{@ThorCam_SaveImageCallback,ih});
uicontrol('Style','pushbutton','String','Save Video','Position',[800 720 100 30],'Callback',{@ThorCam_SaveVideoCallback,ih});
...
function ThorCam_SaveVideoCallback(~,~,ih)
image = get(ih,'CData');
imwrite(image,[datestr(now,'yymmdd_HHMMSS') '_ThorCamImage.avi']);
end
